...
public:
    PointCloudCentroidNode()
        : Node("point_cloud_centroid"), is_ptcld_(false), is_image_(false), display_(true)
    {
        // Publishers
        ...
        processing_timer_ = this->create_wall_timer(
            std::chrono::milliseconds(30), std::bind(&PointCloudCentroidNode::point_cloud_processing, this));

        image_processing_timer_ = this->create_wall_timer(
            std::chrono::milliseconds(30), std::bind(&PointCloudCentroidNode::image_processing, this));
    ...
    void point_cloud_processing()
    {
        if (is_ptcld_) {
            ...
            if (row_id >= 0 && row_id < rows && col_id >= 0 && col_id < cols) {
                ...
                centroid.position.x = static_cast<float>(point[0]);
                centroid.position.y = static_cast<float>(point[1]);
                centroid.position.z = static_cast<float>(point[2]);
