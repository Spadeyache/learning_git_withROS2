...
public:
        ...
        : Node("point_cloud_centroid"), is_image_(false), is_ptcld_(false), display_(true)
    {
        ...
        processing_timer_ = this->create_wall_timer(
            std::chrono::milliseconds(300), std::bind(&PointCloudCentroidNode::image_processing, this));
...
private:
    ...
    bool is_image_, is_ptcld_, display_;
    ...
    void detectAndDisplay(Mat frame)
    {
        ...
        if (largest_face) {
            Point center(largest_face->x + largest_face->width / 2,
                         largest_face->y + largest_face->height / 2);

            ellipse(frame, center, Size(largest_face->width / 2, largest_face->height / 2),
                    0, 0, 360, Scalar(255, 0, 255), 4);

            if (center.y < point_cloud_.rows && center.x < point_cloud_.cols) {
                cv::Vec4f point = point_cloud_.at<cv::Vec4f>(center.y, center.x);
